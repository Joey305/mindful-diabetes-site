...
The editor can supply a **Memovela Resource Blurb**. When it is blank, Mindful Diabetes automatically uses a short curiosity question followed by the article preview summary, so publishing never depends on manually writing a second description.
